...
Products evolve through named stages. The current stage is recorded in `docs/ROADMAP.md`.
...
- The product stage increases (e.g., v1-core → v1 in `ROADMAP.md`), or
...
- `docs/ROADMAP.md` (current stage and areas)
...
> **Note:** `docs/V1_PLAN.md` is SUPERSEDED. Use `docs/ROADMAP.md` for all planning references.

ROADMAP.md is the product roadmap organized as **areas × stages**. It is created by
...
The decomposition agent produces a Stage Map draft; the human authors ROADMAP.md from it.
...
**Inputs:** `state/ideas/idea-NNN.md`, `docs/vision.md`, `docs/ROADMAP.md` (if exists)
**Output:** `state/pre-workflow/NNN-decomp.md`

The agent:
1. Classifies idea size (Bug / Story / Epic / Product)
2. Identifies current product stage from ROADMAP.md (or notes it's missing)
...
6. Flags if ROADMAP.md is missing (human must create it before proceeding with Product mode)
...
`docs/ROADMAP.md`, and recent problems, epics, stories, and captures — so that
...
- Optionally: `docs/ROADMAP.md`, latest `state/problems/`, `state/epics/`, `state/stories/`, `state/capture/`

**Steps:**

1. **Human** (optional, 2–5 min) — Skim `docs/vision.md` and `docs/ROADMAP.md`. Decide whether this run is **Bootstrap** (first catalog) or **Refresh** (existing catalog).
