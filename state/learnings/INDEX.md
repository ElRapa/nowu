...
|------|---------|---------------|-------|------|
| 2026-05-15 | W19 ADR-0011 domain extension model | GAP-003/GAP-005 reframed as bridge/config + know registry extension, not core contract mutation; AP-06 and RE-06 both require typed decision-evidence link manifests; KI-4 needs concrete registry and validation interfaces to keep extension queryable | workflow-process, domain-insight | `state/learnings/session-2026-05-15-w19-domain-extension.md` |
| 2026-05-15 | W9 ADR promotion pass | All 4 ADRs (0007–0010) promoted HYPOTHESIS→INFORMED_ESTIMATE; D-017 met by 3-intake corpus; "implementation gap ≠ design contradiction" unlocked ADR-0008; Status section update is metadata-adjacent (surgical precision required) | workflow-process, domain-insight | `state/learnings/session-2026-05-15-w9-adr-promotion.md` |
| 2026-05-15 | W28 dual-agent comparison | Side-by-side scoring of workflow (T1) vs freeform (T2) across 5 dimensions; GAP-006 divergence analyzed and provisionally resolved as domain-specific for W28 pending monitoring; recommended D-SESS-04 per-gap evidence rows | workflow-process, agent-behavior | `state/learnings/session-2026-05-15-w28-dual-agent.md` |
| 2026-05-15 | W28 RE Domain Bootstrap | Per-gap dual-domain evidence rows made systemic/domain-specific classification defensible; S1 problem-only framing + S2 blindspot checks reduced overclaim risk; 6/7 W27 gaps reappeared cross-domain indicating systemic pressure on K3/W19/W20/K9a | workflow-process, domain-insight | `state/learnings/session-2026-05-15-w28-workflow.md` |
| 2026-05-15 | K3 workflow — MemoryService expansion + KnowAdapter | Keep core contract generic while adapter performs know enum/dataclass conversions; bridge layer must be exempt from global no-know-import checks; mypy strict at external boundaries needs explicit casts/import-ignore strategy | workflow-process, domain-insight, tooling | `state/learnings/session-2026-05-15-k3-workflow.md` |
| 2026-05-15 | K3 dual-agent comparison (T7 vs T8) | Comparison of workflow (T7) and freeform (T8) outputs for K3. Recommends adopting T8 parsing helpers, retaining workflow discipline, and removing broad mypy disables before porting. | agent-behavior, workflow-process, tooling | `state/learnings/session-2026-05-15-k3-dual-agent.md` |
| 2026-05-15 | W27 Dual-Agent Evaluation (nowu vs Oracle) | nowu outputs were template-compliant but Oracle consistently surfaced deeper operational issues; S2 context restrictions (`src/` + USE_CASES visibility) caused highest-impact blind spots; Oracle pre-review before S8 prevented a full rejection loop | agent-behavior, workflow-process, tooling | `state/learnings/session-2026-05-15-dual-agent-evaluation.md` |
